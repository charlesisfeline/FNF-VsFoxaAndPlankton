	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'bubbles' | 'burning-flames' | 'execution':
				credits = 'Original Song made by Foxa the Artist!';
			case 'debug-two':
				credits = "Screw you. You're done.";
			case 'chum-bucket':
				credits = 'Original Song made by JoerOnTheBlower / COOLDUDE2009';
			case 'debug':
				credits = 'Screw you! Original Song made by GreenAnimations! What the hell!';
			case 'tutorial':
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				DiscordClient.changePresence("blueballed >:) - " + detailsText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter());
				#end
				isDead = true;
				return true;
			}
